...
(
  select
    ...
    , Extubated
    , SelfExtubated
  from ventilation_classificatiom_mit_lcp
)
, vd1 as
(
  select
      ...
      , case
          ...
          when MechVent=1 then
            extract(minute from(CHARTTIME - charttime_lag))/60
          else null
        end as ventduration
...
select icustay_id
  ...
  -- , EXTRACT(MINUTE from (max(charttime), min(charttime)))/60 as duration_hours
  , (DATE_PART('EPOCH', max(charttime)) - DATE_PART('EPOCH', min(charttime))) / 3600 as duration_hours
